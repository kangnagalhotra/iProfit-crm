...
-- Phase U: Engagement Score transparency. Independently recomputes the
-- exact same formula as compute_lead_score()/compute_deal_score() above
-- (copied, not calling into them, so the already-live scoring trigger is
-- never touched) but returns the components instead of just the sum, so
-- the UI can show a rep *why* a lead/deal has its score.
